...
class AlarmService:
    ...
    def start_job():
        try:    
            ...
            connection_string = f"DRIVER={driver};SERVER={server};DATABASE={database};UID={uid};PWD={pwd}"
            connection = pyodbc.connect(connection_string)          
            query = f"UPDATE [dbo].[Jobs] SET [LastTimestampStart] = '{now_italian_timezone}' WHERE Job='Alarm'"
            cursor = connection.cursor()         
            cursor.execute(query)
            connection.commit()
        except Exception as e:
    ...
    def refresh_status(bool):
        try:    
            ...
            connection_string = f"DRIVER={driver};SERVER={server};DATABASE={database};UID={uid};PWD={pwd}"
            connection = pyodbc.connect(connection_string)          
            query = f"UPDATE [dbo].[Jobs] SET [LastTimestampEnd] = '{now_italian_timezone}',[Errors] = {1} WHERE Job='Alarm'"
            cursor = connection.cursor()         
            cursor.execute(query)
            connection.commit()
        except Exception as e:
    ...
    def run():
        try:
            ...
            while True:
                ...
                if AlarmService.to_work(now_italian):
                        AlarmService.start_job()    
                        AlarmService.refresh_status(False)
                else:
