...
Kafka.topic_to_manager = Environment.GetEnvironmentVariable("topic_to_manager") ?? config.configuration["topic_to_manager"] ;
...
using (var adminClient = new AdminClientBuilder(adminClientConfig).Build())
{
    ...
    while (!topicCreated)
    {
        try
        {
            ...
            {
                Name = Kafka.topic_to_manager,
                NumPartitions = 1,
                ReplicationFactor = 1
            };

            adminClient.CreateTopicsAsync(new List<TopicSpecification> { topicSpecification }).Wait();
            Console.WriteLine($"Topic '{Kafka.topic_to_manager}' created successfully.");
            log.LogAction($"Topic '{Kafka.topic_to_manager}' created successfully.");
            topicCreated = true;
        }
            ...
            foreach (var innerException in ae.InnerExceptions)
            {
                if (innerException is CreateTopicsException createTopicsException)
                {
                    foreach (var topicError in createTopicsException.Results)
                    {
                        ...
                        {
                            Console.WriteLine($"The topic '{Kafka.topic_to_manager}' already exists.");
                            log.LogAction($"The topic '{Kafka.topic_to_manager}' already exists.");
                            topicCreated = true;
                        }
                        else
                        {
...
{
    c.SwaggerDoc("v1", new OpenApiInfo { Title = "SLAManager WeatherEventNotifier", Version = "v1" });
